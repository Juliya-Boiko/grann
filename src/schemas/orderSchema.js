...
const validationSchema = Yup.object().shape({
  ...
  address: Yup.object().shape({
    city: Yup.string().trim().when('delivery', { is: deliverySets[1].id, then: Yup.string().trim().required("Обов'язкове поле") }),
    street: Yup.string().trim().when('delivery', { is: deliverySets[1].id, then: Yup.string().trim().required("Обов'язкове поле") }),
    build: Yup.string().trim().when('delivery', { is: deliverySets[1].id, then: Yup.string().trim().max(10, 'Максимум 10 символів!').required("Обов'язкове поле") }),
